...
from spacebridgeapp.util.constants import SUBJECT, SEVERITY, CALL_TO_ACTION_URL, CALL_TO_ACTION_LABEL, \
    ...
    ALERT_DASHBOARD_ID, RESULT, ATTACH_TABLE_TOGGLE
...
async def send_push_notification(log: logging.Logger,
    ...
    if sign_keys_response[0] == HTTPStatus.OK:
        ...
        log.exception("Unable to fetch encryption keys with error_code=%s", str(sign_keys_response[0]))
    ...
    for device_id_str in recipient_devices:
        ...
        try:
            ...
            notification_request = notifications.build_notification_request(log,
    ...
    for i in range(len(responses)):
        if isinstance(responses[i], Exception):
            exceptions.append({"Recipient device id": recipient_devices_list[i], "Response": responses[i] })
        else:
            successes.append({"Recipient device id": recipient_devices_list[i], "Status": responses[i].code })
